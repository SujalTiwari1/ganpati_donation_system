...
interface VolunteerRecentDonationsProps {
  donations: VolunteerDonation[];
  isLoading: boolean;
}

export function VolunteerRecentDonations({ donations, isLoading }: VolunteerRecentDonationsProps) {
  return (
    <Card>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b bg-muted/30">
                  ...
                  <th className="hidden px-4 py-3 text-left text-xs font-medium text-muted-foreground md:table-cell">Date</th>
              ...
              <tbody>
                {donations.map((item) => (
                  <tr key={item.id} className="border-b transition-colors hover:bg-muted/40">
                    <td className="px-4 py-3 text-sm font-medium text-foreground">{item.receiptNumber}</td>
                    <td className="px-4 py-3 text-sm text-foreground">{item.donorName}</td>
                    <td className="hidden px-4 py-3 text-sm text-muted-foreground sm:table-cell">{item.buildingName}</td>
